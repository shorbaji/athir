...
fn test_parse_identifier() {
    assert!(Parser::new(["hello".to_string()].into_iter()).read().is_ok());
}
...
//     assert!(Parser::new("(lambda x x)").parse().is_ok());
//     println!("DEFINITION IN BODY? {:?}", Parser::new("(lambda (x) (define y 1))").parse());
//     assert!(Parser::new("(lambda (x) (define y 1))").parse().is_ok());
//     assert!(Parser::new("(lambda (x) (define x 1) 2 (define y 3))").parse().is_err());
// }
