...
use crate::value::{port::Port, Error, Keyword, Procedure, V};
...
impl A {
    ...
    pub fn continuation(f: fn(&R, &R, &R) -> (R, R), r: &R, k: &R) -> R {
        R::new(V::Procedure(Procedure::Continuation {
            f,
            r: r.clone(),
            k: k.clone(),
        }))
    ...
    pub fn continuation_plus(f: fn(&R, &R, &R, &R) -> (R, R), o: &R, r: &R, k: &R) -> R {
        R::new(V::Procedure(Procedure::ContinuationPlus {
            f,
            o: o.clone(),
            r: r.clone(),
            k: k.clone(),
    ...
    pub fn closure(formals: &R, body: &R, env: &R) -> R {
        R::new(V::Procedure(Procedure::Closure {
