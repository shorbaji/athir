...
use crate::alloc::{A, R};
use crate::stdlib::base::{cadr, car, cdr, cons};
use crate::value::{Keyword, Procedure, V};
...
pub fn trampoline(k: &R, e: &R) -> R {
    ...
    loop {
        (k, e) = match k.deref().borrow().deref() {
            ...
            V::Procedure(Procedure::ContinuationNull) => {
                println!();
                std::process::exit(0)
            }
...
pub fn _eval_program(_e: &R, program: &R, r: &R, k: &R) -> (R, R) {
    match program.deref().borrow().deref() {
        ...
        _ => (
            k.clone(),
            A::runtime_error(format!("expected a program as a list of exprs {program:?}")),
        ),
    }
}
...
pub fn eval(e: &R, r: &R, k: &R) -> (R, R) {
    match e.deref().borrow().deref() {
        ...
        V::Pair(car, cdr) => match car.deref().borrow().deref() {
            ...
            V::Keyword(keyword) => (
                k.clone(),
                A::runtime_error(format!("eval error: keyword not implmeneted {keyword:?}")),
            ),
            _ => (
                A::continuation_plus(eval_application, cdr, r, k),
                car.clone(),
            ),
        },
        _ => (
            k.clone(),
            A::runtime_error(format!("eval error: malformed expr {e:?}")),
        ),
    }
}
...
fn eval_set(e: &R, r: &R, k: &R) -> (R, R) {
    ...
    if let V::Symbol(s) = var.deref().borrow().deref() {
        ...
        if let V::Error(_) = lookup(s, r).deref().borrow().deref() {
            return (
                k.clone(),
                A::runtime_error(format!("unbound variable {s:?}")),
            );
        }
    } else {
...
pub fn apply(operator: &R, operands: &R, r: &R, k: &R) -> (R, R) {
    match operator.deref().borrow().deref() {
        ...
        V::Procedure(Procedure::PrimitiveBinary(f, _)) => {
            (k.clone(), f(&car(operands), &cadr(operands)))
        }
        V::Procedure(Procedure::Closure { formals, body, env }) => {
            apply_closure(formals, body, env, operands, r, k)
        }
        V::Procedure(Procedure::PrimitiveOptionalUnary(f, _)) => {
            match operands.deref().borrow().deref() {
                V::Null => (k.clone(), f(None)),
                V::Pair(car, _) => (k.clone(), f(Some(car))),
                _ => (
                    k.clone(),
                    A::runtime_error(format!("not a list {operands:?}")),
                ),
            }
        }
        V::Procedure(Procedure::PrimitiveERK(f, _)) => f(operands, r, k),
        V::Procedure(Procedure::Continuation { f: _, r: _, k: _ }) => {
            (operator.clone(), car(operands))
        }
        V::Procedure(Procedure::ContinuationPlus {
            f: _,
            o: _,
            r: _,
            k: _,
        ...
        _ => (
            k.clone(),
            A::runtime_error(format!("not a procedure {operator:?}")),
        ),
    }
}
...
fn eval_body_after_car(e: &R, o: &R, r: &R, k: &R) -> (R, R) {
    match o.deref().borrow().deref() {
        ...
        _ => (
            k.clone(),
            A::runtime_error(format!("body is not a list {o:?}")),
        ),
    }
}
...
fn lookup(s: &String, r: &R) -> R {
    if let V::Env { map, outer } = r.deref().borrow().deref() {
        if let Some(v) = map.get(s) {
            ...
            A::runtime_error(format!("symbol not found {s:?}"))
        }
    } else {
        A::runtime_error("not an environment".to_string())
    }
