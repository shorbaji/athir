...
pub fn global_env() -> R {
    ...
    map.insert(
        "boolean?".to_string(),
        A::unary(is_boolean, "boolean?".to_string()),
    );
    map.insert(
        "bytevector?".to_string(),
        A::unary(is_bytevector, "bytevector?".to_string()),
    );
    map.insert("char?".to_string(), A::unary(is_char, "char?".to_string()));
    map.insert(
        "eof-object?".to_string(),
        A::unary(is_eof_object, "eof-object?".to_string()),
    );
    map.insert("null?".to_string(), A::unary(is_null, "null?".to_string()));
    map.insert(
        "number?".to_string(),
        A::unary(is_number, "number?".to_string()),
    );
    map.insert("pair?".to_string(), A::unary(is_pair, "pair?".to_string()));
    map.insert("port?".to_string(), A::unary(is_port, "port?".to_string()));
    map.insert(
        "procedure?".to_string(),
        A::unary(is_procedure, "procedure?".to_string()),
    );
    map.insert(
        "string?".to_string(),
        A::unary(is_string, "string?".to_string()),
    );
    map.insert(
        "symbol?".to_string(),
        A::unary(is_symbol, "symbol?".to_string()),
    );
    map.insert(
        "vector?".to_string(),
        A::unary(is_vector, "vector?".to_string()),
    );

    map.insert(
        "display".to_string(),
        A::unary(display, "display".to_string()),
    );
    map.insert(
        "new-line".to_string(),
        A::unary(new_line, "new_line".to_string()),
    );
    map.insert(
        "identity".to_string(),
        A::unary(identity, "identity".to_string()),
    );

    map.insert(
        "call/cc".to_string(),
        A::primitive_erk(call_cc, "call/cc".to_string()),
    );
    map.insert(
        "list".to_string(),
        A::primitive_erk(evlis, "list".to_string()),
    );

    map.insert(
        "read-line".to_string(),
        A::optional_unary(read_line, "read_line".to_string()),
    );
    map.insert(
        "read".to_string(),
        A::optional_unary(read, "read".to_string()),
    );
    map.insert(
        "read-char".to_string(),
        A::optional_unary(read_char, "read_char".to_string()),
    );
    map.insert(
        "peek-char".to_string(),
        A::optional_unary(peek_char, "peek_char".to_string()),
    );
    map.insert(
        "open-input-string".to_string(),
        A::unary(open_input_string, "open_input_string".to_string()),
    );
